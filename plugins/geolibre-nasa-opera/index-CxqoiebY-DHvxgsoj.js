import { _ as toMimeType, a as ModelThrottledError, d as encodeBase64, g as resolveConfigMetadata, i as Model, m as logger, o as ProviderTokenCountError, p as getDefaultExportFromCjs, r as MODEL_DEFAULTS, t as ContextWindowOverflowError, u as defaultModelWarningMessage, y as warnOnce } from "./GeoAgentControl-BK8Q6PEC-QtqA2iOY.js";
//#region node_modules/maplibre-gl-geoagent/dist/index-CxqoiebY.js
...
function requireRetry$1() {
	...
	(function(exports$1) {
		...
		exports$1.operation = function(options) {
			return new RetryOperation(exports$1.timeouts(options), {
...
function getBaseUrl(httpOptions, vertexai, vertexBaseUrlFromEnv, geminiBaseUrlFromEnv) {
	...
	if (!(httpOptions === null || httpOptions === void 0 ? void 0 : httpOptions.baseUrl)) {
		...
		if (vertexai) return (_a2 = defaultBaseUrls.vertexUrl) !== null && _a2 !== void 0 ? _a2 : vertexBaseUrlFromEnv;
...
var GenerateContentResponse = class {
	...
	*/
	get text() {
		...
		if (((_d = (_c = (_b = (_a2 = this.candidates) === null || _a2 === void 0 ? void 0 : _a2[0]) === null || _b === void 0 ? void 0 : _b.content) === null || _c === void 0 ? void 0 : _c.parts) === null || _d === void 0 ? void 0 : _d.length) === 0) return;
...
function resourceName(client, resourceName2, resourcePrefix, splitsAfterPrefix = 1) {
	...
	if (client.isVertexAI()) if (resourceName2.startsWith("projects/")) return resourceName2;
	else if (resourceName2.startsWith("locations/")) return `projects/${client.getProject()}/${resourceName2}`;
	else if (resourceName2.startsWith(`${resourcePrefix}/`)) return `projects/${client.getProject()}/locations/${client.getLocation()}/${resourceName2}`;
	else if (shouldAppendPrefix) return `projects/${client.getProject()}/locations/${client.getLocation()}/${resourcePrefix}/${resourceName2}`;
	else return resourceName2;
	if (shouldAppendPrefix) return `${resourcePrefix}/${resourceName2}`;
...
var Pager = class {
	...
	*/
	hasNextPage() {
		var _a2;
		if (((_a2 = this.params["config"]) === null || _a2 === void 0 ? void 0 : _a2["pageToken"]) !== void 0) return true;
		return false;
	}
};
/**
		...
		if (this.apiClient.isVertexAI()) {
			...
			response = this.apiClient.request({
				...
				return httpResponse.json().then((jsonResponse) => {
					...
					response2.sdkHttpResponse = { headers: httpResponse.headers };
					return response2;
				});
			});
			return response.then((apiResponse) => {
...
var ApiClient = class {
	...
	constructUrl(path2, httpOptions, prependProjectLocation) {
		...
		if (path2 !== "") urlElement.push(path2);
	...
	async apiCall(url, requestInit) {
		...
		return pRetry(runFetch, { retries: ((_a2 = retryOptions.attempts) !== null && _a2 !== void 0 ? _a2 : DEFAULT_RETRY_ATTEMPTS) - 1 });
	}
	getDefaultHeaders() {
		const headers = {};
...
async function throwErrorIfNotOK(response) {
	...
	if (!response.ok) {
		...
		let errorBody;
		if ((_a2 = response.headers.get("content-type")) === null || _a2 === void 0 ? void 0 : _a2.includes("application/json")) errorBody = await response.json();
...
function setMcpUsageHeader(headers) {
	var _a2;
	headers[GOOGLE_API_CLIENT_HEADER] = (((_a2 = headers[GOOGLE_API_CLIENT_HEADER]) !== null && _a2 !== void 0 ? _a2 : "") + ` ${MCP_LABEL}`).trimStart();
}
...
var Live = class {
	...
	*/
	async connect(params) {
		...
		const onopenAwaitedCallback = function() {
			var _a3;
			(_a3 = callbacks === null || callbacks === void 0 ? void 0 : callbacks.onopen) === null || _a3 === void 0 || _a3.call(callbacks);
			onopenResolve({});
		};
		const apiClient = this.apiClient;
		const websocketCallbacks = {
...
function shouldDisableAfc(config) {
	var _a2, _b, _c;
	if ((_a2 = config === null || config === void 0 ? void 0 : config.automaticFunctionCalling) === null || _a2 === void 0 ? void 0 : _a2.disable) return true;
...
function findAfcIncompatibleToolIndexes(params) {
	...
	if (!((_a2 = params === null || params === void 0 ? void 0 : params.config) === null || _a2 === void 0 ? void 0 : _a2.tools)) return afcIncompatibleToolIndexes;
...
var Models = class extends BaseModule {
	constructor(apiClient) {
		...
		this.generateImages = async (params) => {
			return await this.generateImagesInternal(params).then((apiResponse) => {
				...
				if (apiResponse === null || apiResponse === void 0 ? void 0 : apiResponse.generatedImages) for (const generatedImage of apiResponse.generatedImages) if (generatedImage && (generatedImage === null || generatedImage === void 0 ? void 0 : generatedImage.safetyAttributes) && ((_a2 = generatedImage === null || generatedImage === void 0 ? void 0 : generatedImage.safetyAttributes) === null || _a2 === void 0 ? void 0 : _a2.contentType) === "Positive Prompt") positivePromptSafetyAttributes = generatedImage === null || generatedImage === void 0 ? void 0 : generatedImage.safetyAttributes;
		...
		this.list = async (params) => {
			...
			if (this.apiClient.isVertexAI()) {
				if (!actualParams.config.queryBase) if ((_a2 = actualParams.config) === null || _a2 === void 0 ? void 0 : _a2.filter) throw new Error("Filtering tuned models list for Gemini Enterprise Agent Platform (previously known as Vertex AI) is not currently supported");
		...
		this.generateVideos = async (params) => {
			...
			if (!this.apiClient.isVertexAI()) {
				if (((_a2 = params.video) === null || _a2 === void 0 ? void 0 : _a2.uri) && ((_b = params.video) === null || _b === void 0 ? void 0 : _b.videoBytes)) params.video = {
	...
	*/
	...
	async initAfcToolsMap(params) {
		...
		for (const tool of (_b = (_a2 = params.config) === null || _a2 === void 0 ? void 0 : _a2.tools) !== null && _b !== void 0 ? _b : []) if (isCallableTool(tool)) {
			const callableTool = tool;
			...
			return __asyncGenerator(this, arguments, function* () {
				...
				while (remoteCallCount < maxRemoteCalls) {
					...
					if (functionResponses.length > 0) {
						...
						params2.contents = tContents(params2.contents).concat(newContents);
					} else break;
				}
			});
	...
	*/
	async embedContentInternal(params) {
		...
		if (this.apiClient.isVertexAI()) {
			...
			path2 = formatMap(tIsVertexEmbedContentModel(params.model) ? "{model}:embedContent" : "{model}:predict", body["_url"]);
			queryParams = body["_query"];
			delete body["_url"];
			delete body["_query"];
			response = this.apiClient.request({
...
var createPathTagFunction = (pathEncoder = encodeURIPath) => (function path2(statics, ...params) {
	...
	const path3 = statics.reduce((previousValue, currentValue, index) => {
		...
		if (index !== params.length && (value == null || typeof value === "object" && value.toString === ((_c = Object.getPrototypeOf((_b = Object.getPrototypeOf((_a2 = value.hasOwnProperty) !== null && _a2 !== void 0 ? _a2 : EMPTY)) !== null && _b !== void 0 ? _b : EMPTY)) === null || _c === void 0 ? void 0 : _c.toString))) {
			encoded = value + "";
...
var readEnv = (env) => {
	var _a2, _b, _c, _d, _e;
	if (typeof globalThis.process !== "undefined") return ((_b = (_a2 = globalThis.process.env) === null || _a2 === void 0 ? void 0 : _a2[env]) === null || _b === void 0 ? void 0 : _b.trim()) || void 0;
	...
	constructor(apiClient) {
		...
		this.tune = async (params) => {
			...
			if (this.apiClient.isVertexAI()) if (params.baseModel.startsWith("projects/")) {
				...
				if ((_a2 = params.config) === null || _a2 === void 0 ? void 0 : _a2.preTunedModelCheckpointId) preTunedModel.checkpointId = params.config.preTunedModelCheckpointId;
...
async function uploadBlob(file, uploadUrl, apiClient, httpOptions) {
	...
	if (((_a2 = response === null || response === void 0 ? void 0 : response.headers) === null || _a2 === void 0 ? void 0 : _a2[X_GOOG_UPLOAD_STATUS_HEADER_FIELD]) !== "final") throw new Error("Failed to upload file: Upload status is not finalized.");
...
var ERROR_STATUS_MAP = {
	INVALID_ARGUMENT: {
		...
		messagePatterns: /* @__PURE__ */ new Set(["exceeds the maximum number of tokens"])
	},
	RESOURCE_EXHAUSTED: { type: "throttling" },
	UNAVAILABLE: { type: "throttling" }
};
function classifyGoogleError(error) {
	...
	try {
		...
		status = ((_a2 = parsed == null ? void 0 : parsed.error) == null ? void 0 : _a2.status) || "";
		message = ((_b = parsed == null ? void 0 : parsed.error) == null ? void 0 : _b.message) || "";
	} catch {
	...
	if (!config) return;
	if (!config.messagePatterns) return config.type;
	const lowerMessage = message.toLowerCase();
	for (const pattern of config.messagePatterns) if (lowerMessage.includes(pattern)) return config.type;
}
var FINISH_REASON_MAP = { [FinishReason.MAX_TOKENS]: "maxTokens" };
function formatMessages(messages) {
	const contents = [];
	const toolUseIdToName = /* @__PURE__ */ new Map();
	for (const message of messages) for (const block of message.content) if (block.type === "toolUseBlock") toolUseIdToName.set(block.toolUseId, block.name);
	for (const message of messages) {
		const parts = [];
		for (const block of message.content) parts.push(...formatContentBlock(block, toolUseIdToName));
...
function formatContentBlock(block, toolUseIdToName) {
	switch (block.type) {
		case "textBlock": return [{ text: block.text }];
		case "imageBlock": return formatImageBlock(block);
		case "reasoningBlock": return formatReasoningBlock(block);
		case "documentBlock": return formatDocumentBlock(block);
		case "videoBlock": return formatVideoBlock(block);
		case "toolUseBlock": return formatToolUseBlock(block);
		case "toolResultBlock": return formatToolResultBlock(block, toolUseIdToName);
...
function formatImageBlock(block) {
	...
	switch (block.source.type) {
		case "imageSourceBytes": return [{ inlineData: {
			data: encodeBase64(block.source.bytes),
			mimeType
		} }];
		case "imageSourceUrl": return [{ fileData: {
...
function formatDocumentBlock(block) {
	...
	switch (block.source.type) {
		case "documentSourceBytes": return [{ inlineData: {
			data: encodeBase64(block.source.bytes),
			mimeType
		} }];
		case "documentSourceText": return [{ inlineData: {
			data: encodeBase64(new TextEncoder().encode(block.source.text)),
			mimeType
		} }];
		case "documentSourceContentBlock": return block.source.content.map((contentBlock) => ({ text: contentBlock.text }));
...
function formatVideoBlock(block) {
	...
	switch (block.source.type) {
		case "videoSourceBytes": return [{ inlineData: {
			data: encodeBase64(block.source.bytes),
			mimeType
		} }];
...
function formatToolResultBlock(block, toolUseIdToName) {
	const parts = [];
	const output = [];
	for (const c of block.content) switch (c.type) {
		case "textBlock":
		...
		case "imageBlock": {
			...
			if (c.source.type === "imageSourceBytes") parts.push({ inlineData: {
				data: encodeBase64(c.source.bytes),
				mimeType,
				displayName: `image.${c.format}`
			} });
			else logger.warn("source_type=<%s> | only bytes sources supported in gemini tool results", c.source.type);
			break;
		}
		case "documentBlock": {
			const mimeType = toMimeType(c.format) ?? `application/${c.format}`;
			if (c.source.type === "documentSourceBytes") parts.push({ inlineData: {
				data: encodeBase64(c.source.bytes),
				mimeType,
				displayName: c.name
			} });
			else if (c.source.type === "documentSourceText") parts.push({ inlineData: {
				data: encodeBase64(new TextEncoder().encode(c.source.text)),
				mimeType,
				displayName: c.name
			} });
			else logger.warn("source_type=<%s> | only bytes/text sources supported in gemini tool results", c.source.type);
			break;
		}
		case "videoBlock":
...
function mapChunkToEvents(chunk, streamState) {
	const events = [];
	...
	if (content && content.parts) for (const part of content.parts) {
		...
		if ("text" in part && part.text) if ("thought" in part && part.thought === true) {
			if (streamState.textContentBlockStarted) {
				events.push({ type: "modelContentBlockStopEvent" });
				streamState.textContentBlockStarted = false;
			}
	...
	if (finishReason && finishReason !== FinishReason.FINISH_REASON_UNSPECIFIED) {
		if (streamState.textContentBlockStarted) {
			events.push({ type: "modelContentBlockStopEvent" });
			streamState.textContentBlockStarted = false;
		}
...
var GoogleModel = class GoogleModel extends Model {
	/**
	...
	*/
	constructor(options) {
		...
		if (modelConfig.modelId === void 0) warnOnce(logger, defaultModelWarningMessage(MODEL_DEFAULTS.gemini.modelId));
		if (client) this._client = client;
		else {
			const resolvedApiKey = apiKey || GoogleModel._getEnvApiKey();
			if (!resolvedApiKey) throw new Error("Gemini API key is required. Provide it via the 'apiKey' option or set the GEMINI_API_KEY environment variable.");
	...
	*/
	async countTokens(messages, options) {
		...
		try {
			...
			if (response.totalTokens == null) throw new ProviderTokenCountError("Gemini countTokens returned null for totalTokens");
			let totalTokens = response.totalTokens;
			if ((options == null ? void 0 : options.systemPrompt) || (options == null ? void 0 : options.toolSpecs)) totalTokens += await super.countTokens([], {
	...
	*/
	async *stream(messages, options) {
		...
		try {
			...
			for await (const chunk of stream) yield* mapChunkToEvents(chunk, streamState);
			if (streamState.inputTokens > 0 || streamState.outputTokens > 0) yield {
			...
			if (errorType === "contextOverflow") throw new ContextWindowOverflowError(error.message);
			if (errorType === "throttling") throw new ModelThrottledError(error.message, { cause: error });
			throw error;
		}
	}
	/**
	...
	*/
	static _getEnvApiKey() {
		var _a2, _b;
		return (_b = (_a2 = globalThis == null ? void 0 : globalThis.process) == null ? void 0 : _a2.env) == null ? void 0 : _b.GEMINI_API_KEY;
	}
	/**
	...
	_formatRequest(messages, options) {
		...
		if ((options == null ? void 0 : options.systemPrompt) !== void 0) {
			if (typeof options.systemPrompt === "string") {
				if (options.systemPrompt.trim().length > 0) config.systemInstruction = options.systemPrompt;
				...
				if (textBlocks.length > 0) config.systemInstruction = textBlocks.join("");
			}
		}
		if ((options == null ? void 0 : options.toolSpecs) && options.toolSpecs.length > 0) {
			config.tools = [{ functionDeclarations: options.toolSpecs.map((spec) => ({
				name: spec.name,
				description: spec.description,
				parametersJsonSchema: spec.inputSchema
			})) }];
			if (options.toolChoice) {
				if ("auto" in options.toolChoice) config.toolConfig = { functionCallingConfig: { mode: FunctionCallingConfigMode.AUTO } };
				else if ("any" in options.toolChoice) config.toolConfig = { functionCallingConfig: { mode: FunctionCallingConfigMode.ANY } };
				else if ("tool" in options.toolChoice) config.toolConfig = { functionCallingConfig: {
					mode: FunctionCallingConfigMode.ANY,
		...
		if (this._config.params) Object.assign(config, this._config.params);
		return {
